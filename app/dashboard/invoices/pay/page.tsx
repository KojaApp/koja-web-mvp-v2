  ...
  const handleCheckBalance = async () => {
    ...
    try {
      const response = await fetch('/api/dashboard/invoices/pay', {
        ...
        body: JSON.stringify({
          action: 'checkBalance', // Specify the action
          invoiceId: id, // Pass the invoice ID
        }),
      });
      ...
      console.log('Balance checked successfully:', data);
      setHmrcData(data);
      setError(null);
    } catch (error) {
  ...
  const handlePayTFC = async () => {
    ...
    try {
      const response = await fetch('/api/dashboard/invoices/pay', {
        ...
        body: JSON.stringify({
          action: 'payInvoice', // Specify the action
          invoiceId: id, // Pass the invoice ID
        }),
      });
      ...
      alert('Payment successful!'); // Basic feedback for testing
    } catch (error) {
    ...
    <div className="p-6 bg-gray-50 min-h-screen">
      ...
      <header className="mb-6">
        ...
        <p className="text-sm text-gray-500">
          Would you like to pay this invoice now? Click the button to check your account balances and if you have enough cleared funds available to pay the invoice, you can set up your payment to your childcare provider right now. If not, you can set up a payment directly from your bank account, right here.
        </p>
      </header>
